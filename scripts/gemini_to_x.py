...
GOV_TOPICS = [
    "歴史的背景", "経済への影響", "国際比較", "富裕層の動向",
    "一般市民の生活", "税収の使い道", "政府の意図", "未来の展望"
]
...
def generate_article(topic):
    ...
    # プロンプト
    gov_topic = f"""
    あなたは、中立的な立場で政治について解説するAIです。以下の点に注意してください：
    ...
    今日のテーマは「{topic}」です。  
    毎回、異なる切り口から説明してください。100字以内で簡潔かつ具体的に、です・ます調で説明してください。
    """

    try:
        response = genai.GenerativeModel(model_name="gemini-1.5-pro").generate_content(contents=[topic])
